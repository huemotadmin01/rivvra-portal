// ============================================================================
// TimesheetDashboardV2.jsx — ESS dashboard on ds (phase 16, timesheet)
// ============================================================================
// Copied from TimesheetDashboard.jsx. This file is really THREE components: a
// role router, `ContractorDashboard` and `AdminDashboard`. Both dashboards
// carry a social feed — create post, react, comment, delete — which posts
// content visible to the whole org. **Chrome only**, and none of those four
// write paths was triggered.
//
// Migrated: both dashboard headers and the "Unable to load timesheet profile"
// screen. Everything else — celebrations, the feed, the timesheet and
// attendance summaries, the leave balances and every figure — is
// byte-identical.
//
// The role router is preserved exactly, including its comment:
//
//   orgRole = currentOrg ? getAppRole('timesheet') : null
//   effectiveRole = orgRole || (tsRole === 'contractor' ? 'member' : tsRole)
//
// Org membership is the source of truth for access; `ts_users.role` is only a
// fallback when there is no org context. Getting that wrong would show a
// contractor the admin dashboard, so it is copied rather than re-derived.
//
// The contractor header keeps its chip row (employment type, joining date)
// directly under the title — `PageHeader` takes the title only there, because
// the chips are their own row and not a subtitle.
// ============================================================================
...
function ContractorDashboard() {
  ...
  return (
    <div className="p-3 sm:p-6 space-y-4 sm:space-y-6">
      <div>
        <PageHeader title={`Welcome, ${timesheetUser?.fullName || ''}`} />
  ...
  return (
    <div className="p-3 sm:p-6 flex flex-col gap-4 sm:gap-6">
      <div className="order-1">
        <PageHeader
          title={`Welcome, ${timesheetUser?.fullName || ''}`}
          sub={`${timesheetUser?.role === 'admin' ? 'Admin' : 'Manager'} Dashboard`}
        />
      </div>
...
export default function TimesheetDashboardV2() {
  ...
  if (!timesheetUser) {
    return (
      <EmptyState
        ...
        tone="warn"
        title="Unable to load timesheet profile"
        actions={<Button onClick={refetch}>Retry</Button>}
      >
        {typeof error === 'string' ? error : 'The timesheet server may be starting up. Please try again.'}
      </EmptyState>
    );
  }
