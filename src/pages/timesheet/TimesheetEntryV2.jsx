// ============================================================================
// TimesheetEntryV2.jsx — My Timesheet on ds (phase 16, timesheet)
// ============================================================================
// Copied from TimesheetEntry.jsx. **The page header, and nothing else.**
//
// This is the most guard-sensitive page in the app. `isDayDisabled` decides
// whether a day falls inside the consultant's assignment window, and it gates
// FOUR things:
//
//   the grid cells        — can this day be typed into
//   buildEntries()        — "Never persist a day outside this assignment's
//                           window", the comment says, and it means it
//   the footer totals     — so what the consultant sees equals what submits
//   the submit path
//
// **The submit gate is frontend-only.** There is no server-side equivalent, so
// this function is the only thing standing between a mis-click and hours
// persisted against a window the consultant was not assigned to. It is copied
// byte-for-byte, along with `isBeforeProjectStart`, `isAfterProjectEnd`,
// `isAfterLwd` and the assignment-overlap filter that builds `visibleProjects`.
//
// The last-working-date banner is also untouched: it is the only warning a
// leaver gets before typing time they cannot submit.
//
// Nothing was submitted during verification.
// ============================================================================
...
export default function TimesheetEntryV2() {
  ...
  return (
    <div className="p-3 sm:p-6 space-y-4 sm:space-y-6">
      <div className="text-center space-y-3">
        <PageHeader
          title="My Timesheet"
          sub={<span className="hidden sm:block">Enter hours worked per day. Leaves and holidays are managed via the Leave module.</span>}
        />
        {lastWorkingDate && (
