...
import {
  Button, Chip, Field, Input, Modal, Panel, Select, Spinner, Switch, Textarea,
} from '../../components/ds';
...
export default function SignTemplateEditor() {
  ...
  return (
    <div style={{
      display: 'flex', flexDirection: 'column', height: 'calc(100vh - 3.5rem)',
      ...
      {contextMenu && (
        <div
          style={{
            position: 'fixed', zIndex: 50, minWidth: 180, padding: '4px 0',
            background: 'var(--surface-2)', borderRadius: 'var(--r-2)',
            boxShadow: 'inset 0 0 0 1px var(--line), var(--sh-3)',
        ...
        >
          <button
            ...
            style={{
              ...
              padding: '6px 12px', font: `450 13px/1.4 ${FONT}`, color: 'var(--fg-2)',
            }}
            onClick={() => { duplicateField(contextMenu.itemId); setContextMenu(null); }}
          >
            <Plus className="w-3.5 h-3.5" />
            Duplicate
            <span style={{ marginLeft: 'auto', font: `450 10px/1 ${FONT}`, color: 'var(--fg-4)' }}>⌘D</span>
          </button>
          <button
            type="button"
            style={{
              display: 'flex', alignItems: 'center', gap: 8, width: '100%', textAlign: 'left',
              padding: '6px 12px', font: `450 13px/1.4 ${FONT}`, color: 'var(--danger)',
            }}
            onClick={() => { deleteField(contextMenu.itemId); setContextMenu(null); }}
          >
            <Trash2 className="w-3.5 h-3.5" />
            Delete
            <span style={{ marginLeft: 'auto', font: `450 10px/1 ${FONT}`, color: 'var(--fg-4)' }}>⌫</span>
          </button>
        </div>
      )}
      ...
      <header style={{
        ...
        background: 'var(--surface-1)', boxShadow: 'inset 0 -1px 0 var(--line)',
      }}>
        <div className="flex items-center gap-3 min-w-0">
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 'var(--r-2)', color: 'var(--fg-3)',
            }}
            aria-label={isQuickSend ? 'Back to Quick Send' : 'Back to Templates'}
            title={isQuickSend ? 'Back to Quick Send' : 'Back to Templates'}
          >
            <ArrowLeft className="w-4 h-4" />
          </button>
          {isQuickSend && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, font: `450 13px/1 ${FONT}`, color: 'var(--fg-3)' }}>
              <span style={{ color: 'var(--brand-ink)', fontWeight: 550 }}>Quick Send</span>
              <ChevronRight className="w-3.5 h-3.5" />
            </div>
          )}
          {!isQuickSend && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, font: `450 13px/1 ${FONT}`, color: 'var(--fg-3)' }}>
              <span style={{ color: 'var(--brand-ink)', fontWeight: 550 }}>Templates</span>
              <ChevronRight className="w-3.5 h-3.5" />
            </div>
          )}
          ...
          {editingName ? (
            <input
              ...
              style={{
                minWidth: 200, height: 32, padding: '0 12px', border: 'none', outline: 'none',
                borderRadius: 'var(--r-2)', background: 'var(--surface-2)', color: 'var(--fg)',
                font: `450 13.5px/1 ${FONT}`,
            ...
            <button
              ...
              style={{
                font: `550 13.5px/1.4 ${FONT}`, color: 'var(--fg)', maxWidth: 300,
          ...
          {!isQuickSend && (
            <button
              ...
              style={{
                ...
                font: `500 11px/1.4 ${FONT}`, borderRadius: 'var(--r-1)',
                background: editingTags ? 'var(--brand-soft)' : 'var(--surface-2)',
                color: editingTags ? 'var(--brand-ink)' : 'var(--fg-3)',
                boxShadow: `inset 0 0 0 1px ${editingTags ? 'var(--brand-line)' : 'var(--line)'}`,
              }}
          ...
          {isQuickSend && (
            <>
              <Button
                ...
                iconLeft={promoting ? <Spinner size={15} /> : <Bookmark className="w-4 h-4" />}
              >
                Save as Template
              </Button>
              {/* Opens the dialog — it does not send. The send itself is
                  behind the dialog's own confirm button. */}
              <Button
            ...
            <>
              ...
              {(autoSaving || lastSavedAt) && (
                <span
                  style={{
                    ...
                    color: autoSaving ? 'var(--warn-ink)' : 'var(--brand-ink)',
                  }}
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 'var(--r-2)', font: `600 13.5px/1 ${FONT}`, color: 'var(--fg)',
                  background: 'var(--surface-2)',
                  boxShadow: 'inset 0 0 0 1px var(--line), var(--lift)',
                }}
              >
                <X className="w-4 h-4" />
                Cancel
              ...
              <Button
                onClick={handleSave}
                disabled={saving}
                iconLeft={saving ? <Spinner size={15} /> : <Save className="w-4 h-4" />}
              >
                Save
              </Button>
            </>
        ...
        <div style={{
          ...
          <div className="flex items-start gap-3 max-w-3xl">
            ...
            <button
              ...
              style={{ font: `500 12px/1 ${FONT}`, color: 'var(--fg-3)', padding: '4px 8px' }}
              title="Close"
            >
              Done
            </button>
      ...
      <div className="md:hidden flex items-center gap-2 px-4 py-2 shrink-0" style={{
        background: 'var(--surface-1)', boxShadow: 'inset 0 -1px 0 var(--line)',
      }}>
        <button
          ...
          style={{
            ...
            padding: '8px 12px', borderRadius: 'var(--r-2)', font: `550 13px/1 ${FONT}`,
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        {/* ─── Left Sidebar ─────────────────────────────────────── */}
        <aside style={{ background: 'var(--surface-1)', boxShadow: 'inset -1px 0 0 var(--line)' }} className={`w-64 shrink-0 overflow-y-auto ${
              ...
              {FIELD_TYPES.map((ft) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <span className="text-[11px] leading-tight text-center" style={{ color: isPlacing ? 'var(--brand-ink)' : 'var(--fg-3)', fontWeight: isPlacing ? 550 : 450 }}>
                      {ft.label}
                    </span>
                  </button>
                );
          ...
          {/* Roles / Signers */}
          <div className="p-4" style={{ boxShadow: 'inset 0 1px 0 var(--line)' }}>
            <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {signer?.email && (
                          <span className="truncate block text-[10px]" style={{ color: 'var(--fg-4)' }}>{signer.email}</span>
                        )}
                      </div>
                      {isActive && (
                        <MousePointer className="w-3.5 h-3.5 ml-auto shrink-0" style={{ color: 'var(--brand-ink)' }} />
                      )}
                    </button>
                  );
                })}
                  ...
                  {effectiveOrder.map((rid, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="flex-1 min-w-0 truncate text-sm" style={{ color: 'var(--fg)' }}>
                        ...
                        <button
                          ...
                          disabled={isFirst}
                          className="p-1 rounded-md disabled:opacity-30 disabled:cursor-not-allowed" style={{ color: 'var(--fg-3)' }}
                          title="Move up"
                        >
      ...
      {isQuickSend && showSendDialog && (
        <Modal
          open
          onClose={sendingRequest ? undefined : () => setShowSendDialog(false)}
          size="md"
          icon={<Send className="w-5 h-5" style={{ color: 'var(--brand)' }} />}
          title="Send for Signature"
          footer={
            <>
              <Button variant="ghost" onClick={() => setShowSendDialog(false)}>Cancel</Button>
              <span style={{ flex: 1 }} />
              {/* The only control that sends. handleQuickSendSubmit is
                  byte-identical to legacy: it saves the placed fields, then
                  createRequest emails the signers. */}
              <Button
                onClick={handleQuickSendSubmit}
                disabled={sendingRequest}
                iconLeft={sendingRequest ? <Spinner size={15} /> : <Send className="w-4 h-4" />}
              >
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <Field label="Subject" htmlFor="ste-subject">
              <Input
                id="ste-subject"
                value={sendSubject}
                onChange={(e) => setSendSubject(e.target.value)}
                placeholder={`Signature Request - ${templateName}`}
              />
            </Field>

            <Field label="Message (optional)" htmlFor="ste-message">
              <Textarea
                id="ste-message"
                value={sendMessage}
            ...
            <Field label="Valid Until (optional)" htmlFor="ste-validity">
              <Input
                id="ste-validity"
                type="date"
                value={sendValidity}
              ...
              />
              {/* Quick presets — most signing flows want one of these rather
                  than typing a date. Sets validity = today + N days. */}
            ...
            <Field
              ...
              htmlFor="ste-cc"
              hint="CCs receive the completion / refused / cancelled notice. They do not get the signing link."
            >
              <Input
                id="ste-cc"
                type="text"
            ...
            <Field label="Send reminder every" htmlFor="ste-reminder">
              <Select
                id="ste-reminder"
            ...
            <div style={{
              ...
              <p style={{
                ...
                letterSpacing: '.08em', color: 'var(--fg-4)', marginBottom: 6,
              }}>
                Summary
              </p>
              <p style={{ font: `550 13px/1.4 ${FONT}`, color: 'var(--fg)' }}>{templateName}</p>
              <p style={{ font: `450 11.5px/1.5 ${FONT}`, color: 'var(--fg-3)', marginTop: 2 }}>
                {quickSendSigners.length} signer(s) &middot; {signItems.length} field(s) placed &middot; Sequential signing
                {sendReminderDays > 0 ? ` · Reminders every ${sendReminderDays}d` : ''}
              </p>
            </div>
          </div>
        </Modal>
