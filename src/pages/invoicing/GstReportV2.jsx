// ============================================================================
// GstReportV2.jsx — GST report on ds (phase 14, statutory reports)
// ============================================================================
// Copied from GstReport.jsx. **Chrome only.** Migrated: the page header, the
// Sync-GSTN button, the granularity segmented control, the FY picker, and the
// error / loading states. Everything below them is byte-identical:
//
//   - every GST figure and its table
//   - the `HowToRead` primer and every `TermHint` (the readability pass that
//     shipped as cd5ef6e5 — this migration is not the place to re-word them)
//   - `StatutoryFilingModal` and `StatutoryRecordsModal`
//   - `downloadStatutoryCsv` — the GSTR-1 export, which carries the FX and
//     SEZ fixes. It was NOT triggered during verification: it produces a
//     filing artefact, and a layout pass has no business generating one.
//
// The outer `min-h-screen … max-w-7xl mx-auto` wrapper is dropped because the
// v2 shell already supplies page padding and the 1360px content column;
// keeping it would have nested a second, narrower column inside the first.
// ============================================================================
          ...
          <>
            {data?.gstn?.enabled && (
              <Button
                ...
                iconLeft={syncing ? <Spinner size={13} /> : <RefreshCw size={13} />}
            ...
            {/* Granularity is a segmented control, not tabs: it re-slices the
                same report rather than navigating between views. */}
            <div style={{ display: 'inline-flex', borderRadius: 'var(--r-2, 12px)', overflow: 'hidden', boxShadow: 'inset 0 0 0 1px var(--line)' }}>
              {GRANULARITIES.map((g) => (
                <button
                  key={g.key}
                  type="button"
                  ...
                  style={{
                    padding: '8px 12px', font: "600 11.5px/1 'Inter', system-ui, sans-serif",
                    background: granularity === g.key ? 'var(--info-soft)' : 'transparent',
                    color: granularity === g.key ? 'var(--info)' : 'var(--fg-3)',
                  }}
                >
                  {g.label}
                </button>
