...
function KPIStrip({ kpis }) {
  ...
  if (rows.length === 0) {
    return <EmptyState compact title="No invoicing activity yet" />;
  }

  const cells = [
    { key: 'totalInvoiced', label: 'Total Invoiced', color: 'var(--info)' },
    { key: 'totalCollected', label: 'Collected', color: 'var(--brand)' },
    { key: 'totalOutstanding', label: 'Outstanding', color: 'var(--warn)' },
    { key: 'totalOverdue', label: 'Overdue', color: 'var(--danger)' },
  ];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      {rows.map((row) => (
        <div key={row.currency}>
          {/* The per-currency heading only appears for multi-currency tenants,
              exactly as before — a single-currency org must not gain a stray
              "INR" label it never had. */}
          {rows.length > 1 && (
            <div style={{
              font: `600 10px/1.3 ${FONT}`, textTransform: 'uppercase', letterSpacing: '.08em',
          ...
          <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))' }}>
            {cells.map((cell) => (
              <Stat
                key={cell.key}
                label={cell.label}
                value={formatCurrency(row[cell.key] || 0, row.currency)}
                color={cell.color}
              />
            ))}
          </div>
...
export default function InvoicingDashboardV2() {
