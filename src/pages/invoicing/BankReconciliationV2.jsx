// ============================================================================
// BankReconciliationV2.jsx — Bank reconciliation on ds (phase 14, invoicing)
// ============================================================================
// Copied from BankReconciliation.jsx. **Chrome only.** Migrated: the page
// header and its New Statement toggle. Byte-identical: `createBankStatement`,
// `reconcileLine`, `getReconciliationSuggestions`, the statement form, the
// match/suggestion UI and every figure.
//
// `reconcileLine` marks a bank line as matching a recorded payment — a
// financial assertion, not a UI state. It was not triggered, nor was
// `createBankStatement`.
//
// ⚠️ VERIFICATION LIMIT: **no bank statement exists on staging.** Only the
// empty state renders, so the statement list, the line table and the
// suggestion matcher were never exercised — same category as the GSTR-2B
// table in batch 7. Their correctness rests on the static diff alone.
// Importing a statement to work around that would have created financial
// records on staging, which batch 4 showed cannot reliably be cleaned up.
// ============================================================================
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <PageHeader
        ...
        sub="Match bank statement lines with recorded payments"
        actions={
          <Button onClick={() => setShowForm(!showForm)} iconLeft={showForm ? <X size={16} /> : <Plus size={16} />}>
