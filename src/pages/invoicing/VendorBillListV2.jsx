// ============================================================================
// VendorBillListV2.jsx — Vendor + Employee bills on ds (phase 14, money pass)
// ============================================================================
// Serves BOTH /invoicing/bills and /invoicing/employee-bills (via `mode`).
//
// This is the only WRITE surface in the batch: the AI import runs
// extractVendorBill → createInvoice → uploadAttachment, i.e. it creates a real
// vendor bill. So the migration here is deliberately narrower than the others —
// **page chrome only**:
//
//   migrated : header + actions, the filter toolbar, loading and empty states
//   untouched: the whole AI extraction flow and its drop zone, every handler
//              above `return (`, the CSV export, and the ResizableTable block
//              with all of its money cells
//
// The reasoning is the standing one for anything that writes: prove it by diff,
// don't rewrite it. Restyling a drop zone is not worth any chance of disturbing
// a path that creates financial records, and the extraction flow was never
// exercised in verification for the same reason — a run would have created a
// real bill on staging and burned an AI call.
//
// `ResizableTable` is kept for the same reasons as InvoiceListV2: it is already
// themed under `.ds-shell` by legacy-bridge.css (which names this page), and it
// owns persisted column widths, sticky columns and the footer slot.
// ============================================================================
...
import {
  Plus, Loader2, FileText, ChevronLeft, ChevronRight,
  X, ArrowUpDown, Building2, Sparkles, Download,
} from 'lucide-react';
import {
  Button, EmptyState, Field, PageHeader, PageSpinner, Panel, SearchInput, Spinner, Tabs,
} from '../../components/ds';
...
export default function VendorBillListV2({ mode = 'vendor' } = {}) {
    ...
    <div className="p-4 sm:p-6 space-y-5">
      <PageHeader
        ...
        actions={
          <>
            ...
            {!isEmployeeMode && (
              <Button onClick={() => navigate(orgPath('/invoicing/bills/new'))} iconLeft={<Plus size={16} />}>
                Create Bill
              </Button>
            )}
          </>
      ...
      {aiStep === 'choose' && aiPayload && (
        <VendorChoiceModal
          ...
          createLabel="Create vendor & bill"
        />
      )}

      {/* Tabs */}
      {/* Same ds Tabs + invoicing accent as InvoiceListV2 — these are sibling
          pages in one app and the legacy strips had drifted apart. */}
      <Tabs
        tabs={TABS.map(t => ({ key: t.key, label: t.label, count: tabCounts[t.key] }))}
        value={activeTab}
        onChange={setActiveTab}
        accent="var(--a-invoice)"
      />

      {/* Toolbar: search + FY filter + employee pickers + clear + export */}
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12, flexWrap: 'wrap' }}>
        <Field label="Search" style={{ flex: 1, minWidth: 200, maxWidth: 360 }}>
          <SearchInput
            value={searchInput}
        ...
        {isEmployeeMode && (
          <>
            <Field label="Submitted by" style={{ flex: 1, minWidth: 200, maxWidth: 260 }}>
              <EmployeePicker
            ...
            <Field label="Approved by" style={{ flex: 1, minWidth: 200, maxWidth: 260 }}>
              <EmployeePicker
        ...
        {hasActiveFilters && (
          <Button variant="ghost" size="sm" onClick={clearAllFilters} iconLeft={<X size={12} />} title="Clear all filters">
            Clear filters
          </Button>
        )}
        <div style={{ flex: 1 }} />
      ...
      {/* Table */}
      {loading ? (
        <PageSpinner minHeight="40vh" />
      ) : bills.length === 0 ? (
        <Panel>
          <EmptyState
            icon={<FileText size={22} />}
            title={search || activeTab ? 'No bills match your filters' : emptyHint}
            actions={!search && !activeTab && !isEmployeeMode ? (
              <Button variant="secondary" onClick={() => navigate(orgPath('/invoicing/bills/new'))}>
                Create your first bill
              </Button>
            ) : null}
          >
            {!search && !activeTab && isEmployeeMode
