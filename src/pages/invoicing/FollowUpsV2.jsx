// ============================================================================
// FollowUpsV2.jsx — Overdue follow-ups on ds (phase 14, invoicing)
// ============================================================================
// Copied from FollowUps.jsx. **Chrome only**, and the reason is the strictest
// in the pass: `sendFollowUp` **emails the customer** about an overdue
// invoice. That is on the never-trigger list, so it was not fired, and the
// send path plus its config (`updateFollowUpConfig`) are byte-identical.
//
// Migrated: the page header and its Refresh action. Everything else —
// StatutoryDuesCard, the overdue table, the per-invoice send controls, the
// escalation tiers and the log — is untouched.
// ============================================================================
...
export default function FollowUpsV2() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <PageHeader
        title="Follow-ups"
        sub="Manage overdue invoice follow-ups and escalations"
        actions={
          <Button variant="ghost" onClick={fetchOverdue} disabled={loading} iconLeft={<RefreshCw size={16} className={loading ? 'animate-spin' : ''} />}>
            Refresh
          </Button>
        }
      />
