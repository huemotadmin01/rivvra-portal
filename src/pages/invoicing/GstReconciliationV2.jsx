// ============================================================================
// GstReconciliationV2.jsx — GSTR-2B recon on ds (phase 14, statutory)
// ============================================================================
// Copied from GstReconciliation.jsx. **Chrome only**, and more conservatively
// than the other statutory pages, because this one has FOUR write paths:
//
...
//
// None was triggered. All four handlers, the India gate, the bucket
// definitions, the match logic, the CSV column order and the `HowToRead`
// primer are byte-identical. Migrated: the page header, the return-period
// picker, the upload / re-reconcile buttons and the India-gate screen.
//
// ⚠️ VERIFICATION LIMIT — read before trusting this page.
// **No period on staging has a GSTR-2B uploaded** (checked 2026-04 through
// 2026-07 and 2025-12: every one returns zero rows). So the bucket cards,
// the reconciliation table and every figure in it were NEVER RENDERED during
// verification — only the empty state was. Their correctness rests on the
// static diff alone.
//
// I did not fabricate a 2B to work around this: uploading one is a statutory
// write, and batch 4 established that staging rows of this kind cannot
// reliably be removed afterwards. Before this page is enabled for a
// production org, look at it on a period that actually has a 2B.
// ============================================================================
...
import { Button, EmptyState, Field, Input, PageHeader, Spinner } from '../../components/ds';
...
export default function GstReconciliationV2() {
  ...
  if (indiaBlocked || companyCountry !== 'IN') {
    return (
      <EmptyState
        ...
        actions={<Button variant="secondary" onClick={() => navigate(orgPath('/invoicing/dashboard'))} iconLeft={<ArrowLeft size={15} />}>Back</Button>}
      >
        GSTR-2B reconciliation is available only for companies registered in India.
        Switch to an Indian company to use it.
      </EmptyState>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <PageHeader
        ...
        sub="Match vendor bills against your GSTR-2B to confirm ITC eligibility."
        actions={
          <>
            ...
            <Button
              ...
              iconLeft={uploading ? <Spinner size={15} /> : <Upload size={15} />}
            >
              Upload GSTR-2B JSON
            </Button>
            {hasImportForPeriod && (
              <Button variant="secondary" onClick={() => runReconcile(period)} disabled={reconciling || uploading}>
