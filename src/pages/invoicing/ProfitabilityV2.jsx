// ============================================================================
// ProfitabilityV2.jsx — Net profit dashboard on ds (phase 14, invoicing)
// ============================================================================
// Copied from Profitability.jsx. **Chrome only**, for three reasons stacked on
// one page:
//
//  1. It WRITES, and both writes are consequential. `saveProfitAdjustment`
//     records a manual expense against a month — it moves net profit.
//     `setProfitabilityAccess` grants or revokes a user's access to net-profit
//     figures. Neither was triggered; see the KB over-grant incident for why
//     access toggles are not something to exercise casually on shared data.
//  2. It is owner-gated. The `!allowed` screen is preserved (moved to ds
//     `EmptyState`, copy verbatim) and still sits after all hooks.
//  3. It is one of only two files in the app importing recharts. There is no
//     ds chart primitive, so the chart is left exactly as it is rather than
//     hand-rolled into one.
//
// The methodology paragraph under the title is kept VERBATIM. It is the page's
// only defence against being read against the Customer Invoices list — it
// states GST-exclusion, service-period recognition and ECB conversion, and
// says outright that the totals will not match. Shortening it to fit a tidier
// header would remove the warning, not the discrepancy.
// ============================================================================
...
{/* Scoped override for recharts' SVG. The chart's tick fills, zero line
            and hover cursor are hard-coded dark-only values (#9ca3af, #6b7280,
            #374151, white @3%). In light theme the X ticks measured 2.29:1 —
            below AA — and the zero line and cursor were near-invisible.

            The recharts props are deliberately NOT edited: a CSS `fill`
            declaration beats an SVG presentation attribute, so a scoped
            stylesheet themes all four without touching the chart config, and
            the component diff stays clean. `var()` does not resolve inside a
            presentation attribute, which is why this has to be CSS. */}
          {/* display:contents — the wrapper must not join the layout, or it
              breaks ResponsiveContainer's height chain and the chart renders
              at zero height. Caught by the chart disappearing entirely. */}
          <div className="pf-chart" style={{ display: 'contents' }}>
            <style>{`
              /* The tick TEXT carries .recharts-cartesian-axis-tick-value; the
                 group is .recharts-cartesian-axis-tick-label. Targeting the
                 text class directly is the version-tolerant option. */
...
export default function ProfitabilityV2() {
  ...
  if (!allowed) {
    return (
      ...
      >
        Net profit is restricted to the organization owner.
      </EmptyState>
    );
  }
      ...
      <PageHeader
        title={
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
            <TrendingUp size={20} style={{ color: 'var(--brand-ink)' }} />
            Profitability
          </span>
        }
        sub={
          <>
            ...
            {/* This paragraph is the page's whole defence against being
                mis-read against the Customer Invoices list. Kept verbatim. */}
            <span style={{ display: 'block', marginTop: 6, font: 'var(--t-small)', color: 'var(--fg-4)', maxWidth: '72ch' }}>
              GST-excluded; revenue recognised by <span style={{ color: 'var(--fg-2)' }}>service period</span> (each line&apos;s
              service month), not invoice date. Foreign currencies are converted to {data?.currency || 'the base currency'}
              {' '}at <span style={{ color: 'var(--fg-2)' }}>reference (ECB) rates</span> on the payment date (paid) or invoice date (unpaid)
        ...
        actions={
          <>
            ...
            <Button variant="secondary" size="sm" onClick={() => setShowAdj(true)} iconLeft={<SlidersHorizontal size={15} />}>
              Other expenses
            </Button>
          </>
        }
