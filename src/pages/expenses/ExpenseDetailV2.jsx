...
import { Button, Chip, Field, Modal, Spinner, Textarea } from '../../components/ds';

const FONT = "'Inter', system-ui, sans-serif";

/* ============================================================================
 * ExpenseDetailV2 — expense claim on ds (phase 9)
 * ============================================================================
 * The page this project twice declined to migrate, because it is a money
 * surface on two counts: it does FX arithmetic in the page, and it owns the
 * approval and reimbursement transitions. Migrated now on explicit
 * instruction, under the strictest discipline used so far.
 *
 * NOTHING that produces a number or moves a claim between states is touched.
 * Verified by diff, not by eye:
 *
 *   - `lineConverted()` — `Math.round(amt * rate * 100) / 100`, and the
 *     `sameCcy` short-circuit that forces rate 1 when the line currency
 *     equals the claim currency, so a same-currency line can never be
 *     scaled by a stale rate.
 *   - `totalAmount` — the reduce over `lineConverted`.
 *   - every `formatCurrency` call site, argument for argument. Note the
 *     middle one deliberately formats against `line.originalCurrency`, not
 *     the claim currency: that cell shows what was actually spent.
 *   - submit / resubmit / withdraw / approve / reject / cancel / delete /
 *     sync / reimburse / reverse — all untouched.
 *
 * `STATUS_META` keeps `approved` on the SAME warn tone as `submitted`, not a
 * success tone. Bills are created on approval, so a happy-path claim never
 * lingers in `approved` — that state means the bill failed to sync, and it
 * has to read as "needs attention" rather than "done".
 * ========================================================================== */
import {
...
// Tone by meaning. `reimbursed` has no violet token; it takes `info` and the
// label carries the distinction — inventing an accent for one status would be
...
// `color` is now a token, read as an inline style rather than a class.
const EVENT_LABEL = {
  created:   { icon: FileText,     color: 'var(--fg-4)',    label: 'Created' },
  submitted: { icon: Send,         color: 'var(--warn)',    label: 'Submitted for approval' },
  withdrawn: { icon: Undo2,        color: 'var(--fg-3)',    label: 'Withdrawn back to draft' },
  approved:  { icon: CheckCircle2, color: 'var(--info)',    label: 'Approved' },
  synced:    { icon: CheckCircle2, color: 'var(--brand)',   label: 'Synced to Employee Bill' },
  reimbursed:{ icon: CheckCircle2, color: 'var(--a-sign)',  label: 'Reimbursed' },
  reimbursement_reversed: { icon: Undo2, color: 'var(--warn)', label: 'Reimbursement reversed' },
  rejected:  { icon: XCircle,      color: 'var(--danger)',  label: 'Rejected' },
  resubmitted:{ icon: Send,        color: 'var(--warn)',    label: 'Resubmitted for approval' },
  commented: { icon: MessageSquare,color: 'var(--fg-3)',    label: 'Comment' },
};
...
function ConfirmModalBody({ title, body, confirmLabel, confirmTone = 'rivvra', onConfirm, onCancel, requireReason = false, busy }) {
  ...
  // The caller's tone names map to ds Button variants. Only `red` is
  // destructive; the rest are ordinary confirms. `blue`/`emerald` were
  // decorative in the legacy modal and carry no separate meaning.
  const variant = confirmTone === 'red' ? 'danger' : 'primary';
  ...
  return (
    <Modal
      ...
      size="sm"
      tone={confirmTone === 'red' ? 'danger' : 'brand'}
      icon={confirmTone === 'red' ? <AlertTriangle size={16} /> : null}
      title={title}
      footer={
        <>
          <span style={{ flex: 1 }} />
          ...
          <Button
            ...
            iconLeft={busy ? <Spinner size={12} /> : null}
          >
            {confirmLabel}
          </Button>
        </>
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {body && (
          <p style={{ font: `450 13px/1.6 ${FONT}`, color: 'var(--fg-2)' }}>{body}</p>
        )}
        {requireReason && (
          <Field label="Reason" required htmlFor="expense-confirm-reason">
            <Textarea
              id="expense-confirm-reason"
              value={reason}
...
export default function ExpenseDetailV2() {
  const { id } = useParams();
