...
export default function UserDetail() {
  ...
  async function handleSaveRateLimits() {
    ...
    try {
      ...
      const res = await api.updateMemberRateLimits(member.userId, payload);
      if (res.success) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        {/* Organization Role */}
        <Panel title="Organization Role" icon={<Shield size={14} />}>
          {isEditing && editData ? (
        ...
        {/* Authentication */}
        <Panel title="Authentication" icon={<Lock size={14} />}>
          {isEditing && editData ? (
        ...
        {/* Member Email */}
        {isEditing && editData && (
          <Panel title="Member Email" icon={<Mail size={14} />}>
        ...
        <div className="lg:col-span-2">
          <Panel title="App Access" icon={<Users size={14} />}>
            {isEditing && editData ? (
              <div className="space-y-2">
        ...
        <Panel title="Linked Employees" icon={<Link2 size={14} />}>
        ...
        {allCompanies && allCompanies.length > 1 && (
          <Panel title="Companies" icon={<Building2 size={14} />}>
            {isEditing && editData ? (
        ...
        {member.appAccess?.outreach?.enabled && memberRateLimits && (
          <Panel title="Rate Limits" icon={<Mail size={14} />}>
            {editingRateLimits ? (
              <div className="space-y-3">
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
