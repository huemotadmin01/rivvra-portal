  ...
  return (
    <div className="mb-3">
      <div className="flex items-center justify-between mb-1">
        <span className="text-xs font-semibold text-ds-fg-2">{label}</span>
      ...
      <pre className="text-xs text-ds-fg-2 bg-ds-surface-3 border border-ds-line rounded-lg p-3 whitespace-pre-wrap break-words">{text}</pre>
    </div>
  );
}
  ...
  return (
    <>
      <button
        type="button"
        onClick={generate}
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-ds-line-strong text-xs font-semibold text-ds-fg-2 hover:text-ds-fg hover:bg-ds-surface-3 transition-colors"
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setOpen(false)}>
          <div className="bg-ds-surface-2 border border-ds-line rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto p-5" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-semibold text-ds-fg">Sourcing strings</h3>
              <button type="button" onClick={() => setOpen(false)} className="p-1.5 rounded-lg text-ds-fg-3 hover:text-ds-fg hover:bg-ds-surface-3"><X size={16} /></button>
            </div>
            {loading ? (
              <div className="flex items-center gap-2 text-sm text-ds-fg-3 py-8 justify-center"><Loader2 size={16} className="animate-spin" /> Generating from the job's requirements…</div>
            ) : error ? (
              ...
              <>
                ...
                <p className="text-[11px] text-ds-fg-4 mt-2">Generated from this job's title, required skills and JD. Edit the job and reopen to regenerate.</p>
              </>
            ) : null}
          </div>
        </div>
